...
graph LR
    ...
    CF -->|/api/*| ALB[ALB]
    CF -->|/r/*| ALB
    ALB --> EKS[EKS<br/>Go API Pod]
    EKS -->|IRSA| DDB[(DynamoDB)]
...
> 詳細な図: [architecture.drawio](docs/architecture.drawio) / [network.drawio](docs/network.drawio)
...
graph TD
    ...
    CI --> GoTest[go test / vet / build]
    CI --> WebBuild[pnpm build]

    Merge[Merge to main] --> CD[GitHub Actions CD]
    CD --> Docker[Docker Build → ECR Push]
    CD --> S3Deploy[pnpm build → S3 Sync → CF Invalidation]
    Docker --> Kustomize[Update kustomization.yaml]
    Kustomize --> GitPush[Git Push]
    GitPush --> ArgoCD[ArgoCD detects change]
    ArgoCD --> EKS[Deploy to EKS]
```

> 詳細な図: [cicd.drawio](docs/cicd.drawio)
